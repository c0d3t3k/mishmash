...
import { processImages, initializeModel, getModelInfo } from "./process";
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900" onPaste={handlePaste}>
            ...
            {!isIOS && (
              <div className="flex items-center gap-4">
                ...
                <select
                  ...
                  className="bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md px-3 py-1 text-sm text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  disabled={!isWebGPU}
                >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className={`grid ${images.length === 0 ? 'grid-cols-2 gap-8' : 'grid-cols-1'}`}>
          ...
          <div className={images.length === 0 ? '' : 'w-full'}>
            <div
              {...getRootProps()}
              className={`p-8 mb-8 border-2 border-dashed rounded-lg text-center cursor-pointer transition-colors duration-300 ease-in-out bg-white dark:bg-gray-800
                ${isDragAccept ? "border-green-500 bg-green-50 dark:bg-green-900/20" : ""}
                ${isDragReject ? "border-red-500 bg-red-50 dark:bg-red-900/20" : ""}
                ${isDragActive ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20" : "border-gray-300 dark:border-gray-600 hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20"}
              ...
              <div className="flex flex-col items-center gap-2">
                {isLoading || isModelSwitching ? (
                  ...
                  <>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-500">or click to select files</p>
                  </>
                )}
              </div>
            </div>
            ...
            {images.length === 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border dark:border-gray-700">
                <h3 className="text-xl text-gray-700 dark:text-gray-300 font-semibold mb-4">No image? Try one of these:</h3>
